...
class SocketRouterScope:
    ...
    def __init__(self, event_detail: str, param_name: str, paramter: Parameter):
        self._event_detail = event_detail
        self._param_name = param_name
        self._paramter = paramter
        if self._paramter.kind == self._paramter.VAR_POSITIONAL or self._paramter.kind == self._paramter.VAR_KEYWORD:
            raise ValueError("Cannot use * or ** in the event function parameters.")
        self._default = self._paramter.default if self._paramter.default != self._paramter.empty else None
        ...
        elif self._paramter.annotation is SocketRequest:
            self._create_scope = self._create_request_scope()
        elif self._paramter.annotation is WebSocket:
            self._create_scope = self._create_websocket_scope()
        elif issubclass(self._paramter.annotation, BaseModel):
            self._create_scope = self._create_model_scope(self._paramter.annotation)
        else:
            self._create_scope = self._create_data_scope(self._paramter.annotation)
    ...
    def _create_depends_scope(self, default: Depends) -> TScopeCreator:
        ...
        for param_name in depend_params:
            depend_param_creators[param_name] = SocketRouterScope(self._event_detail, param_name, depend_params[param_name])
    ...
    def _create_data_scope(self, annotation: type) -> TScopeCreator:
        def create_scope(req: SocketRequest):
            if req.data is None or isinstance(req.data, list):
                return req.data

            if self._param_name not in req.data and self._param_name not in req.route_data:
                return None

            raw_data = req.data.get(self._param_name, req.route_data.get(self._param_name))
