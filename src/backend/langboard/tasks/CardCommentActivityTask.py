    ...
    async with ActivityTaskHelper.use_helper(ProjectActivity) as helper:
        activity_history = {
            **await _get_default_history(helper, project, card, comment),
            "changes": {
                "before": {"content": await ActivityHistoryHelper.convert_to_python(helper.db, old_content)},
                "after": {"content": await ActivityHistoryHelper.convert_to_python(helper.db, comment.content)},
            },
        }
        activity = await helper.record(
...
async def _get_default_history(helper: ActivityTaskHelper, project: Project, card: Card, comment: CardComment):
    ...
    result = await helper.db.exec(
        helper.db.query("select").table(target_model, with_deleted=True).where(target_model.column("id") == target_id)
    )
    user_or_bot = cast(User | Bot, result.first())
    history = {
        **await helper.create_project_default_history(project, card),
        "comment": await ActivityHistoryHelper.create_card_comment_history(helper.db, comment, user_or_bot),
    }
