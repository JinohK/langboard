...
class ProjectBotPublisher(BaseSocketPublisher):
    ...
    async def scheduled(project: Project, schedule: tuple[BotSchedule, BaseBotScheduleModel]):
        target_table = BotUtils.get_target_table_by_bot_model("schedule", schedule[1].__class__)
        model = {"target_table": target_table, "schedule": {**schedule[0].api_response(), **schedule[1].api_response()}}
    ...
    async def rescheduled(project: Project, schedule: BaseBotScheduleModel, updated: dict[str, Any]):
        target_table = BotUtils.get_target_table_by_bot_model("schedule", schedule.__class__)
        model = {"target_table": target_table, "uid": schedule.get_uid(), "updated": updated}
        topic_id = project.get_uid()
        publish_model = SocketPublishModel(
            topic=SocketTopic.BoardSettings,
            topic_id=topic_id,
    ...
    async def scope_created(project: Project, bot_scope: BaseBotScopeModel):
        scope_table = BotUtils.get_target_table_by_bot_model("scope", bot_scope.__class__)
        topic_id = project.get_uid()
    ...
    async def scope_conditions_updated(project: Project, bot_scope: BaseBotScopeModel):
        scope_table = BotUtils.get_target_table_by_bot_model("scope", bot_scope.__class__)
        model = {"scope_table": scope_table, "uid": bot_scope.get_uid(), "conditions": bot_scope.conditions}
        topic_id = project.get_uid()
        publish_model = SocketPublishModel(
            topic=SocketTopic.BoardSettings,
            topic_id=topic_id,
    ...
    @staticmethod
    async def log_stack_added(project: Project, bot_log: BotLog, stack: BotLogMessage):
        topic_id = project.get_uid()
        model = {
            "uid": bot_log.get_uid(),
