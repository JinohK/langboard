...
from models.bases import BaseBotLogModel, BaseBotScheduleModel, BaseBotScopeModel, BotTriggerCondition
from ..service import ServiceHelper
from .ModelUtils import get_models_by_base_class
...
class BotUtils:
    ...
    def get_bot_model_class(class_type: _TBotTypeName, target_table: str) -> _TBaseBotTypeClass | None:
        if target_table not in BotUtils.AVAILABLE_TARGET_TABLES:
            return None

        base_model_class = BotUtils.AVAILABLE_BOT_TABLES.get(class_type)
        if not base_model_class:
            return None

        model_classes = get_models_by_base_class(base_model_class)
    ...
    def get_target_model_by_param(
        ...
        target_schedule_class = BotUtils.get_bot_model_class(class_type, target_table)
        target_class = BotUtils.AVAILABLE_TARGET_TABLES.get(target_table)
    ...
    def get_target_model_by_bot_model(class_type: _TBotTypeName, bot_model: _TBaseBotType) -> _TAvailableTargets | None:
        ...
        target_table = BotUtils.get_target_table_by_bot_model(class_type, bot_model.__class__)
        if not target_table:
            return None
    ...
    def get_scope_model_classes_by_condition(condition: BotTriggerCondition):
        model_classes = get_models_by_base_class(BaseBotScopeModel)
    ...
    def get_target_table_by_bot_model(class_type: _TBotTypeName, bot_model: _TBaseBotTypeClass) -> str | None:
        ...
        target_table = bot_model.__tablename__.split(f"_{BotUtils.SUFFIXES[class_type]}")[0]
        if target_table not in BotUtils.AVAILABLE_TARGET_TABLES:
            return None
