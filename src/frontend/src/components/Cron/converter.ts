...
import { convertStringToNumber, dedup, range, setError, sort } from "@/components/Cron/utils";
...
export function setValuesFromCronString(
    ...
    internalValueRef: RefObject<string>,
    firstRender: bool,
    shortcuts: Shortcuts,
...
export function getCronStringFromValues(
    ...
    const parsedArray = parseCronArray([newMinutes, newHours, newMonthDays, newMonths, newWeekDays], humanizeValue, dropdownsConfig);
 ...
 */
export function partToString(cronPart: number[], unit: Unit, humanize?: bool, leadingZero?: LeadingZero, clockFormat?: ClockFormat) {
    let retval = "";
 ...
 * Format the value
 */
export function formatValue(value: number, unit: Unit, humanize?: bool, leadingZero?: LeadingZero, clockFormat?: ClockFormat) {
 ...
 */
function parseCronArray(cronArr: number[][], humanizeValue: bool | undefined, dropdownsConfig: DropdownsConfig | undefined) {
    return cronArr.map((partArr, idx) => {
        const unit = UNITS[idx];
        const parsedArray = parsePartArray(partArr, unit);
        const dropdownOption: DropdownConfig | undefined = dropdownsConfig?.[unit.type];

        return partToString(parsedArray, unit, dropdownOption?.humanizeValue ?? humanizeValue);
    });
}
