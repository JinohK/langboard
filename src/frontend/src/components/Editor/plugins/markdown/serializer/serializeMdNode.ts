...
import type { MdElementType, MdLeafType, MdNodeType, MdNodeTypes } from "@/components/Editor/plugins/markdown/serializer/types";
import { isLeafNode } from "@/components/Editor/plugins/markdown/serializer/utils";

type MarkFormats = Record<
    "bold" | "boldItalic" | "boldItalicStrikethrough" | "code" | "italic" | "strikethrough" | "subscript" | "superscript" | "underline",
    string[] | string | null
>;
...
export interface SerializeMdOptions {
     ...
     *
     * @default ['disc', 'circle', 'square', 'todo']
     */
...
export function serializeMdNode(node: MdElementType | MdLeafType, opts: SerializeMdOptions) {
    ...
    // Serialize children
    if (!isLeafNode(node)) {
        const isMentionParagraph = !!node.children.find((child) => (child as unknown as Record<string, unknown>).type === "mention");
        children = node.children
            .map((c: MdElementType | MdLeafType, index, all) => {
                ...
                if (isIndentList) {
                    ...
                    const isList = isLeafNode(c) ? false : (LIST_TYPES as string[]).includes(c.type || "");
                    ...
                    listDepth = (LIST_TYPES as string[]).includes((c as MdElementType).type || "") ? listDepth + 1 : listDepth;
                }
                /**
                 ...
                 */
                if ((!isLeafNode(c) && Array.isArray(node.children)) || isMentionParagraph) {
                    ignoreParagraphNewlineProp = true;
                }
    ...
    if (isLeafNode(node)) {
        ...
        if (children !== opts.breakTag) {
            ...
            if (markFormats.boldItalicStrikethrough && leaf[nodes.strikethrough.type] && leaf[nodes.bold.type] && leaf[nodes.italic.type]) {
                children = retainWhitespaceAndFormat(children, markFormats.boldItalicStrikethrough);
                ...
                if (markFormats.subscript && leaf[nodes.subscript.type]) {
                    children = retainWhitespaceAndFormat(children, markFormats.subscript);
                }
                if (markFormats.superscript && leaf[nodes.superscript.type]) {
                    children = retainWhitespaceAndFormat(children, markFormats.superscript);
                }
                ...
                if (markFormats.strikethrough && leaf[nodes.strikethrough.type]) {
                    children = retainWhitespaceAndFormat(children, markFormats.strikethrough);
                }
                if (markFormats.code && leaf[nodes.code.type]) {
                    children = retainWhitespaceAndFormat(children, markFormats.code);
                }
                if (markFormats.hightlight && leaf[nodes.highlight.type]) {
                    children = retainWhitespaceAndFormat(children, markFormats.hightlight);
                }

                const leafOptions = Object.values({ ...nodes, ...customNodes }).find((n) => {
                    return leaf[n.type];
                });
