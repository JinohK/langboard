...
export function CheckboxStatic({ className, ...props }: { checked: bool; className?: string; style?: React.CSSProperties }) {
    return (
        <button
            className={cn(
            ...
            {...props}
        >
            <div className={cn("flex items-center justify-center text-current")}>{props.checked && <Check className="size-4" />}</div>
        </button>
    );
}
