...
export function CodeBlockCombobox() {
    ...
    const items = languages.filter(
        (language) =>
            !value || language.label.toLowerCase().includes(value.toLowerCase()) || language.value.toLowerCase().includes(value.toLowerCase())
    );

    return (
        <Popover.Root open={open} onOpenChange={setOpen}>
            <Popover.Trigger asChild>
                <Button size="sm" variant="ghost" className="h-5 justify-between px-1 text-xs" aria-expanded={open} role="combobox">
            ...
            <Popover.Content className="w-auto p-0">
                <Command.Root shouldFilter={false}>
                    <Command.Input value={value} onValueChange={(value) => setValue(value)} placeholder={t("editor.Search language...")} />
