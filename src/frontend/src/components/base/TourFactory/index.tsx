...
function TourFactory<T extends string>(order: T[]) {
    ...
    function TourPortal() {
        ...
        const currentElement = useMemo(() => ctx.nodes.get(order[ctx.current] ?? ""), [ctx]);
        ...
        const closest = (): React.CSSProperties => {
            ...
            const isCloseToRight = currentElementRect.right > window.innerWidth - width;

            if (isCloseToLeft) {
                return {
                    left: currentElementRect.x + currentElementRect.width,
                    top: currentElementRect.y - height / 2 + currentElementRect.height / 2,
                };
            }
        ...
        return createPortal(
            <div
                id="tour"
                className={cn("pointer-events-auto fixed left-0 top-0 h-screen w-screen transition-none", !ctx.show ? "invisible" : "visible")}
            >
                <div
                    ref={ref}
                    className={cn("absolute z-50 transition-all duration-500 ease-in-out")}
                    style={{
                        ...closest(),
                    }}
                >
    ...
    return {
        Provider: function Provider(props: TourProps) {
            ...
            const getNextIndex = (currIndex: number, nextDiff: number): number => {
                ...
                if (lookAheadIndex >= order.length || lookAheadIndex < 0) return currIndex;

                if (!nodes.current.has(order[lookAheadIndex])) return getNextIndex(lookAheadIndex, nextDiff);
            ...
            return (
                <tourContext.Provider
                    value={{
                        ...
                        show,
                        next: () => {
                            setCurrent((state) => Math.min(getNextIndex(state, 1), order.length - 1));
                        },
                        previous: () => {
        ...
        Focus: function Focus(props: TourFocusProps<T>) {
            const ctx = useContext(tourContext);
            return (
                <div
                    ref={(divRef) => {
