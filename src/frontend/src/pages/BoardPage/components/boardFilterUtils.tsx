...
export const filterTask = (filters: IFilterMap, task: IBoardTask) => {
    const keyword = filters.keyword?.join(",");
    if (!keyword) {
        return true;
    }

    return task.title.includes(keyword) || task.members.some((member) => filterMember(filters, member));
};

export const filterTaskMember = (filters: IFilterMap, members: User.Interface[], task: IBoardTask, currentUser: IAuthUser) => {
    if (!filters.members?.length) {
        return true;
    }

    if (filters.members.includes("none") && !task.members.length) {
        return true;
    }
    ...
    for (let i = 0; i < filters.members.length; ++i) {
        ...
        if (task.members.some((member) => member.email === user.email && member.username === user.username)) {
            return true;
        }
    }
...
export const filterTaskRelationships = (filters: IFilterMap, task: IBoardTask) => {
    if (!filters.parents?.length && !filters.children?.length) {
        return true;
    }

    const oppositeRelationships: Record<keyof IBoardTask["relationships"], keyof IBoardTask["relationships"]> = {
        parents: "children",
        children: "parents",
    };

    const filter = (relationship: keyof IBoardTask["relationships"]) => {
        const opposite = oppositeRelationships[relationship];
        return (
            filters[relationship]!.some((oppositeUID) => task.relationships[opposite].includes(oppositeUID)) ||
            filters[relationship]!.includes(task.uid)
        );
    };
