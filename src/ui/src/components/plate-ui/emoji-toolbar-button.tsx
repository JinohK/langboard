...
import { type EmojiCategoryList, type EmojiIconList, type GridRow, EmojiSettings, i18nProps } from "@platejs/emoji";
    ...
    return (
        <div key={row.id} className="flex" data-index={row.id}>
            {row.elements.map((emojiId, index) => (
                <EmojiButton key={emojiId} onMouseOver={onMouseOver} onSelect={onSelectEmoji} emoji={emojiLibrary.getEmoji(emojiId)} index={index} />
            ))}
        </div>
    );
});
...
function EmojiPickerContent({
    ...
    UseEmojiPickerType,
    "emojiLibrary" | "i18n" | "isSearching" | "onMouseOver" | "onSelectEmoji" | "refs" | "searchResult" | "settings" | "visibleCategories"
>) {
            ...
            .map(({ id: categoryId }) => {
                ...
                return (
                    <div key={categoryId} ref={section.root} style={{ width: getRowWidth }} data-id={categoryId}>
                        <div className="backdrop-blur-xs sticky -top-px z-10 bg-popover/90 p-1 py-2 text-sm font-semibold">
...
function EmojiPickerSearchAndClear({ clearSearch, i18n, searchValue }: Pick<UseEmojiPickerType, "clearSearch" | "i18n" | "searchValue">) {
    return (
        <div className="flex items-center text-foreground">
            <div className={cn("absolute left-2.5 top-1/2 z-10 flex size-5 -translate-y-1/2 items-center justify-center text-foreground")}>
