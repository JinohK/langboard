import type { Axis, Edge, InternalConfig, Side } from "@/core/helpers/dnd/auto-scroll/internal-types";
...
import { type HitboxSpacing } from "@/core/helpers/dnd/auto-scroll/unsafe-overflow/types";

function makeGetHitbox({ axis, side }: { axis: Axis; side: Side }) {
    return function hitbox({ clientRect, overflow, config }: { clientRect: DOMRect; overflow: HitboxSpacing; config: InternalConfig }): {
...
export const getHitbox: {
    [Key in Edge]: (args: { clientRect: DOMRect; overflow: HitboxSpacing; config: InternalConfig }) => {
        insideOfEdge: DOMRect;
        outsideOfEdge: DOMRect;
    };
} = {
