...
import { clearEngagementHistory, clearUnusedEngagements } from "@/core/helpers/dnd/auto-scroll/shared/engagement-history";
...
const schedulers: Map<(args: MonitorArgs<AllDragTypes>) => CleanupFn, Scheduler<AllDragTypes>> = new Map();

export function getScheduler<DragType extends AllDragTypes>(monitor: (args: MonitorArgs<DragType>) => CleanupFn): Scheduler<DragType> {
...
function makeScheduler<DragType extends AllDragTypes>(monitor: (args: MonitorArgs<DragType>) => CleanupFn): Scheduler<DragType> {
    ...
    function loop(timeLastFrameFinished: DOMHighResTimeStamp) {
        ...
        clearUnusedEngagements(() => {
            callbacks.forEach((onFrame) => onFrame({ underUsersPointer, latestArgs, timeSinceLastFrame }));
        });
