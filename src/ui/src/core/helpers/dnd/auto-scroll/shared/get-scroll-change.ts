...
import type { Axis, Edge, EngagementHistoryEntry, InternalConfig } from "@/core/helpers/dnd/auto-scroll/internal-types";
import { axisLookup } from "@/core/helpers/dnd/auto-scroll/shared/axis";
import { getPercentageInRange } from "@/core/helpers/dnd/auto-scroll/shared/get-percentage-in-range";
import { mainAxisSideLookup } from "@/core/helpers/dnd/auto-scroll/shared/side";

// We want a consistent scroll speed across devices, regardless of framerate
function getMaxScrollChange({ timeSinceLastFrame, config }: { timeSinceLastFrame: number; config: InternalConfig }): number {
...
function getDistanceDampening({
    ...
    const maxSpeedBuffer = hitbox[mainAxis.size] * config.maxScrollAtPercentageRemainingOfHitbox[edge];
